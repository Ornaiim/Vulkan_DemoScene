...
std::vector<std::string> MeshModel::LoadMaterials(const aiScene* scene)
{
	...
	// Go through each materials and copy its texture file name (if it exists)
	for (size_t i = 0; i < scene->mNumMaterials; i++)
	{
		// Get the materiam
		aiMaterial* material = scene ->mMaterials[i];

		// Initialise the textue to empty string (will be replaced if texture exists)
		textureList[i] = "";
...
std::vector<Mesh> MeshModel::LoadNode(VkPhysicalDevice newPhysicalDevice, VkDevice newDevice, VkQueue transferQueue, VkCommandPool transferCommandPool, aiNode* node, const aiScene* scene, std::vector<int> matToTex)
{
	...
	// Go through each node attached to this node and load it, then append their meshes to this nodes's mesh list
