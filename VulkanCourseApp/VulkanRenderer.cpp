...
int VulkanRenderer::init(GLFWwindow* newWindow)
{
	...
	try {
		...
		Mesh firstMesh = Mesh(mainDevice.physicalDevice, mainDevice.logicalDevice, graphicsQueue, graphicsCommandPool, &meshVertices, &meshIndices, createTexture("sheep.jpg"));
		Mesh secondMesh = Mesh(mainDevice.physicalDevice, mainDevice.logicalDevice, graphicsQueue, graphicsCommandPool, &meshVertices2, &meshIndices, createTexture("sheep2.jpg"));
...
void VulkanRenderer::createInstance()
{
	...
	// Information about the application ifself
...
void VulkanRenderer::populateDebugMessengerCreateInfo(VkDebugUtilsMessengerCreateInfoEXT& createInfo)
{
	...
	createInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT; // Specifies all types of severities for the callback to be called
...
void VulkanRenderer::setupDebugMessenger()
{
	...
	populateDebugMessengerCreateInfo(callbackCreateInfo);

	// Create debug callback with custom create function
	if (CreateDebugUtilsMessengerEXT(instance, &callbackCreateInfo, nullptr, &debugMessenger) != VK_SUCCESS)
	{
...
void VulkanRenderer::createLogicalDevice()
{
	...
	for (int queueFamilyIndex : queueFamilyIndices)
	{
		...
		float priority = 1.0f;
		queueCreateInfo.pQueuePriorities = &priority;						// Vulkans needs to know how to handle multiple queues, so decide priority (1 = highest priority)
	...
	// Physical device feartures the logical device will be using
...
void VulkanRenderer::createSwapChain()
{
	...
	// If old swapchain been destroyed and this one replaces it, then link old one to quickly hand over responsabilites
...
void VulkanRenderer::createDescriptorSetLayout()
{
	...
	vpLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;								// Shader stage to bind to to
	...
	// Create descriptor Set Layout given bindings
	...
	VkDescriptorSetLayoutCreateInfo inputLayoutCreateUnfo = {};
	inputLayoutCreateUnfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_LAYOUT_CREATE_INFO;
	inputLayoutCreateUnfo.bindingCount = static_cast<uint32_t>(inputBindings.size());
	inputLayoutCreateUnfo.pBindings = inputBindings.data();

	// Create descriptor set layout
	result = vkCreateDescriptorSetLayout(mainDevice.logicalDevice, &inputLayoutCreateUnfo, nullptr, &inputSetLayout);
	if (result != VK_SUCCESS)
	{
		throw std::runtime_error("Failed to create a Descriptor Set Layout!");
	}
...
void VulkanRenderer::createGraphicsPipeline()
{
	...
	// How the data for a single vertex (including info such as position, colour, texture coords, normals, etc) is as whole
	...
	inputAssembly.primitiveRestartEnable = VK_FALSE;										// Allow overriding of "strip" topoligy to start new primitives
	...
	multisamplingCreateInfo.rasterizationSamples = VK_SAMPLE_COUNT_1_BIT;					// Number of samples to user per fragment


	// -- BLENDING --
	// Blending decides how to blend a new colour being wrtten to a fragment, with the old value
	...
	// Blending uses equation: (srcColourBlendFactor * new colour) colorBlendOp (dstColorBlendFactor * old colour)
	...
	depthStencilCreateInfo.depthBoundsTestEnable = VK_FALSE;							// Depth bound test: does the depth value exist between to bounds
...
void VulkanRenderer::createSynchronisation()
{
	...
	for (size_t i = 0; i < MAX_FRAME_DRAWS; i++)
	{
		if (vkCreateSemaphore(mainDevice.logicalDevice, &semaphoreCreateInfo, nullptr, &imageAvailable[i]) != VK_SUCCESS || vkCreateSemaphore(mainDevice.logicalDevice, &semaphoreCreateInfo, nullptr, &renderFinished[i]) != VK_SUCCESS ||
...
void VulkanRenderer::createDescriptorPool()
{
	...
	vpPoolSize.descriptorCount = static_cast<uint32_t>(modelDUniformBuffer.size());*/
...
{	
	...
	// Update all of descriptor set buffer bindins
	for (size_t i = 0; i < swapChainImages.size(); i++)
	{
		...
		vkCmdBeginRenderPass(commandBuffers[currentImage], &renderPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);
			...
			for (size_t j = 0; j < modelList.size(); j++)
			{
				...
				for (size_t k = 0; k < thisModel.getMeshCount(); k++)
				{
					...
					vkCmdBindVertexBuffers(commandBuffers[currentImage], 0, 1, vertexBuffers, offsets);		// Command to bind vertex buffer before drawing with it
...
{	
	...
	// Check if all the validation layers exist in the availableLayers list
...
QueueFamilyIndices VulkanRenderer::getQueueFamilies(VkPhysicalDevice device)
{
	...
	for (const auto& queueFamily : queueFamilyList)
	{
		...
		// Queue can be multiple types defined through bitfield. Need to bitwise AND with VK_*_BIT to check if has required type
		if (queueFamily.queueCount > 0 && queueFamily.queueFlags & VK_QUEUE_GRAPHICS_BIT)
		{
			indices.graphicsFamily = i;			// If queue family is valide, then get index
		}
...
VkSurfaceFormatKHR VulkanRenderer::chooseBestSurfaceFormat(const std::vector<VkSurfaceFormatKHR>& formats)
{
	...
	// If restricted, seach for optimal format
...
VkImageView VulkanRenderer::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags)
{
	...
	// Surbresources allow the view to view only a part of an image
	viewCreateInfo.subresourceRange.aspectMask = aspectFlags;			// Which aspect of image to view (e.g. COLOR_BIT for viewing colour)
	viewCreateInfo.subresourceRange.baseMipLevel = 0;					// Start mipmiap level to view from
...
int VulkanRenderer::createMeshModel(std::string modelFile)
{
	...
	// Conversion from the materials list IDs to out descriptor array IDs
